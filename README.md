...
Примечание: `requirements.txt` теперь включает `pydantic` для валидации JSON-ответов LLM.
...
## Недавние улучшения (robustness)

- LLM-вызовы: добавлен метод с retry и экспоненциальным backoff для устойчивости к временным ошибкам.
- Валидация ответов агентов: ответы LLM теперь проверяются простыми Pydantic-схемами перед использованием.
- Хранилище: запись `DealObject` стала атомарной (запись в временный файл и `os.replace`) чтобы избежать повреждения данных при сбоях.

Эти изменения повышают надёжность пайплайна при отсутствии API-ключей или при нестабильных сетевых условиях.
